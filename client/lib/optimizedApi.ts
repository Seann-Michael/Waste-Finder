...
export const queryKeys = {
  ...
  reference: {
    ...
    locationTypes: () =>
      [...queryKeys.reference.all, "location-types"] as const,
  },
  analytics: {
    all: ["analytics"] as const,
    stats: () => [...queryKeys.analytics.all, "stats"] as const,
    views: (locationId: string) =>
      [...queryKeys.analytics.all, "views", locationId] as const,
  },
};
...
export const optimizedQueries = {
  ...
  async getLocations(
    ...
    const fieldMaps = {
      ...
      standard:
        "id, name, address, city, state, zip_code, location_type, latitude, longitude, rating, review_count, is_active",
      full: `
        *,
...
export const useOptimizedLocations = (
  options: {
    fields?: "minimal" | "standard" | "full";
    filters?: any;
    limit?: number;
    ...
    setInterval(() => {
      ...
      this.queryClient.invalidateQueries({
        predicate: (query) => {
          ...
          const isStale =
            Date.now() - lastFetch > CACHE_CONFIG.locations.staleTime;
...
export const useDebouncedLocationSearch = (
  searchTerm: string,
  filters: any = {},
) => {
