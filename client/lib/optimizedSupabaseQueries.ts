...
const FIELD_TEMPLATES = {
  location: {
    ...
    minimal: 'id, name, city, state, location_type, latitude, longitude, rating',
...
export const locationQueries = {
  ...
  async getLocations(options: {
    ...
    return queryOptimizer.memoizedQuery(cacheKey, async () => {
  ...
  async getLocation(id: string, fields: keyof typeof FIELD_TEMPLATES.location = 'full') {
    ...
    return queryOptimizer.memoizedQuery(cacheKey, async () => {
      ...
      if (error) throw error;
      return data;
    }, 600000); // 10 minutes cache for single locations
  },
  
  // Batch fetch multiple locations efficiently
  async getLocationsBatch(ids: string[], fields: keyof typeof FIELD_TEMPLATES.location = 'standard') {
  ...
  async searchLocations(searchTerm: string, options: {
    limit?: number;
    fields?: keyof typeof FIELD_TEMPLATES.location;
...
export const referenceQueries = {
  ...
  async getAllReferenceData() {
    ...
    return queryOptimizer.memoizedQuery(cacheKey, async () => {
      const [debrisTypesResult, paymentTypesResult] = await Promise.allSettled([
        supabase
          .from('debris_types')
          .select('id, name, category, price_per_ton, price_per_load, description')
          .order('category, name'),
      ...
      const debrisTypes = debrisTypesResult.status === 'fulfilled' 
        ? debrisTypesResult.value.data || []
        : [];
      
      const paymentTypes = paymentTypesResult.status === 'fulfilled'
    ...
    }, 3600000); // 1 hour cache for reference data
  },
  ...
  async getLocationGeography() {
    ...
    return queryOptimizer.memoizedQuery(cacheKey, async () => {
      ...
      return Object.entries(geography).map(([state, cities]: [string, any]) => ({
        state,
...
export const analyticsQueries = {
    ...
    return queryOptimizer.memoizedQuery(cacheKey, async () => {
      ...
      if (error) {
        ...
        const statsPromises = [
          ...
          supabase.from('locations').select('state', { count: 'exact' }).eq('is_active', true),
          supabase.from('reviews').select('id', { count: 'exact' })
        ];
        
        const [locationsResult, statesResult, reviewsResult] = await Promise.allSettled(statsPromises);
        
        return {
          totalLocations: locationsResult.status === 'fulfilled' ? locationsResult.value.count || 0 : 0,
          totalStates: statesResult.status === 'fulfilled' ? new Set((statesResult.value.data || []).map((l: any) => l.state)).size : 0,
          totalReviews: reviewsResult.status === 'fulfilled' ? reviewsResult.value.count || 0 : 0
        };
      }
  ...
  async trackLocationView(locationId: string, userAgent?: string, referrer?: string) {
