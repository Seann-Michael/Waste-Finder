...
export const optimizeLCP = () => {
  ...
  const preloadCriticalResources = () => {
    const criticalImages = document.querySelectorAll(
      'img[data-priority="high"], [data-hero-image]',
    );
    const criticalFonts = [
  ...
  const optimizeImages = () => {
    ...
    images.forEach((img) => {
      ...
      if (!element.loading) {
        element.loading =
          element.dataset.priority === "high" ? "eager" : "lazy";
      }
...
export const initializeCoreWebVitals = () => {
  ...
  const { addPassiveListeners, deferNonCriticalJS, scheduleTask } =
    optimizeFID();
  const { addAspectRatios, reserveSpaceForContent, optimizeFontLoading } =
    optimizeCLS();
