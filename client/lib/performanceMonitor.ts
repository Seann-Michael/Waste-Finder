  ...
  private trackPageLoad() {
    ...
    window.addEventListener('load', () => {
      ...
      if ('performance' in window && 'getEntriesByType' in performance) {
        ...
        this.metrics.bundleSize = jsResources.reduce((sum, resource) => 
  ...
  public trackApiCall(url: string) {
    ...
    return {
        ...
        if (url.includes('/api/')) {
          ...
          this.metrics.cacheHitRate = (this.metrics.cacheHitRate || 0) + (success ? 1 : 0);
        }
      }
    };
  }
  
  public trackUserInteraction(type: 'click' | 'search' | 'form' | 'navigation') {
    const startTime = performance.now();
...
export const trackUserInteraction = (type: 'click' | 'search' | 'form' | 'navigation') => {
