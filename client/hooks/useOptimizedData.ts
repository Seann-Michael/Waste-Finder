...
import {
  ...
  analyticsQueries,
  batchOperations,
} from "../lib/optimizedSupabaseQueries";
...
export const queryKeys = {
  locations: {
    ...
    search: (term: string) =>
      [...queryKeys.locations.all, "search", term] as const,
    batch: (ids: string[]) =>
      [...queryKeys.locations.all, "batch", ids.sort()] as const,
  },
  reference: {
...
export const useLocation = (
  id: string,
  fields: "minimal" | "standard" | "full" | "admin" = "full",
) => {
  const queryClient = useQueryClient();
  ...
  useEffect(() => {
    if (query.data && id) {
      ...
      analyticsQueries
        .trackLocationView(id, navigator.userAgent, document.referrer)
        .catch(console.warn);
    }
  }, [query.data, id]);
...
export const useLocationsBatch = (
  ids: string[],
  fields: "minimal" | "standard" | "full" = "standard",
) => {
  return useQuery({
...
export const useCreateLocation = () => {
  ...
  return useMutation({
    mutationFn: (newLocation: any) =>
      batchOperations
        .createLocationsBatch([newLocation])
        .then((data) => data[0]),
...
export const useUpdateLocation = () => {
  ...
  return useMutation({
    mutationFn: ({ id, updates }: { id: string; updates: any }) =>
      batchOperations
        .updateLocationsBatch([{ id, updates }])
    ...
    onMutate: async ({ id, updates }) => {
      ...
      if (previousLocation) {
        queryClient.setQueryData(queryKeys.locations.detail(id), {
          ...previousLocation,
          ...updates,
        });
      }
...
export const usePrefetchLocation = () => {
  ...
  return useCallback(
    (id: string, fields: "minimal" | "standard" | "full" = "full") => {
      queryClient.prefetchQuery({
        queryKey: queryKeys.locations.detail(id),
        queryFn: () => locationQueries.getLocation(id, fields),
    ...
    const interval = setInterval(() => {
      queryClient.refetchQueries({
        predicate: (query) => {
          ...
          const isStale =
            Date.now() - state.dataUpdatedAt > (state.staleTime || 0);
